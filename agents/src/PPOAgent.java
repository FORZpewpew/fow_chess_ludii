...
/**
 * PPOAgent — wraps the trained PPO policy checkpoint via a Python subprocess.
 *
 * <p>On {@link #initAI} it spawns {@code ppo/venv/bin/python ppo/ppo_server.py}
 * (relative to the JVM working directory, i.e. the ludii project root),
 * waits for the "READY" handshake, then on each {@link #selectAction} call it:
 * <ol>
 *   <li>Builds the 128-float observation from the Ludii {@link Context}.</li>
 *   <li>Builds the 4096-bool legal-action mask.</li>
 *   <li>Sends a JSON line to the subprocess stdin.</li>
 ...
 */
public class PPOAgent extends AI {

    private static final String PYTHON_BIN       = "ppo/venv/bin/python";
    private static final String SERVER_SCRIPT    = "ppo/ppo_server.py";
    ...
    public void initAI(final Game game, final int playerID) {
        ...
        try {
            final ProcessBuilder pb = new ProcessBuilder(PYTHON_BIN, SERVER_SCRIPT);
            ...
            final Thread errFwd = new Thread(() -> {
                try (BufferedReader br = new BufferedReader(
                    ...
                    while ((ln = br.readLine()) != null) {
                        System.err.println("[ppo_server] " + ln);
                    }
            ...
            while (System.currentTimeMillis() < deadline) {
                if (!serverProcess.isAlive()) {
                    throw new RuntimeException(
                            "PPO server process died before printing READY (exit code "
            ...
            if (!"READY".equals(handshake)) {
                throw new RuntimeException(
                        "PPO server did not send READY within " + (READY_TIMEOUT_MS / 1000)
                        + " s; got: " + handshake);
            }
            System.out.println("[PPOAgent] Python inference server ready (player " + playerID + ").");

        } catch (final Exception e) {
            throw new RuntimeException("PPOAgent.initAI failed: " + e.getMessage(), e);
        }
    }
            ...
            final int maxDepth)
    {
        ...
        try {
            ...
            if (response == null) {
                System.err.println("[PPOAgent] Server stdout closed — using fallback.");
                return moves.get(0);
            }
            ...
            System.err.println("[PPOAgent] selectAction error: " + e.getMessage());
            return moves.get(0);
        }
    }
